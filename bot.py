...
def is_debug_channel_join(msg):
    return (
        msg["type"] == "member_joined_channel"
        and msg["channel"] == DEBUG_CHANNEL_ID
        and msg["channel_type"] == "C"
    )
...
def parse_join(message):
    ...
    if is_team_join(m) or is_debug_channel_join(m):
        ...
        conversation = requests.post(
            "https://slack.com/api/conversations.open?token="
            + TOKEN
            + "&users="
            + user_id,
            data=None,
        ...
        post_message_response = requests.post(
